...
class MPChan(object):
    ...
    def __init__(self, data_frame, is_outage = False):
        """
        ...
        if is_outage:
            ...
            for ipath in range(self.npath):
                ...
                # print(f'****************** {decimal.Decimal.from_float(data_frame.at[0, str(path_id)+DataConfig.path_dly])}')
                self.dly[ipath] = data_frame.at[0, 
                        str(path_id)+DataConfig.path_dly]
                # print(f'****************** {decimal.Decimal.from_float(self.dly[ipath])}')
    ...
    def compute_RM_error(self, ref_chan, valdn_chan):
        '''
        ...
        for i_path in range(ref_chan.npath):
            ...
            phase_ = 2*np.pi -(2*np.pi*DataConfig.freq*ref_chan.dly[i_path])%(2*np.pi)
            arbitrary_phase = np.deg2rad(ref_chan.phase[i_path]) - phase_
            
            phase_from_est_dist = 2*np.pi - (2*np.pi*est_dist*DataConfig.freq/PhyConst.light_speed_remcom) % (2*np.pi)
        ...
        Hest, Eest = comp_cmplx_gain(gain_ls, phase_ls)
        Htrue, Etrue = comp_cmplx_gain(-valdn_chan.pl, np.deg2rad(valdn_chan.phase))
    ...
    def compute_PWA_error(self, ref_chan):
        """
        ...
        Hest, Eest = comp_cmplx_gain(gain_ls, phase_ls)
        Htrue, Etrue = comp_cmplx_gain(-self.pl, np.deg2rad(self.phase))
        rel_err = np.abs(Hest-Htrue)**2/Eest
    ...
    def compute_const_error(self, ref_chan):
        """
        ...
        """
        Href, Eref = comp_cmplx_gain(-ref_chan.pl, np.deg2rad(ref_chan.phase))
...
def comp_cmplx_gain(gain, phase):
    ...
    for i in range(len(gain)):
        ...
        h += g * np.exp(phase[i]*1j) 
        Eavg += g**2

    return h, Eavg
            
       
def test_channl(df_root, ref_df_root, test_df_root, test_channel_num, i_lamdba, results, mape_results):
    ...
    if np.isnan(channel_df.at[0, DataConfig.paths_number]):
        mp_chan = MPChan(channel_df, True)
        print("displaced pair outage")
    else:
        mp_chan = MPChan(channel_df)
        if np.isnan(ref_channel_df.at[0, DataConfig.paths_number]):
            ...
            if np.isnan(test_channel_df.at[0, DataConfig.paths_number]):
                ...
                test_chan = MPChan(test_channel_df)
                relerr_rm, mape_rm = mp_chan.compute_RM_error(ref_chan, test_chan)
                relerr_pwa, mape_pwa = test_chan.compute_PWA_error(ref_chan)
                relerr_const, mape_const = test_chan.compute_const_error(ref_chan)
                    
                results[i_lamdba, test_channel_num, 0] = relerr_pwa
                results[i_lamdba, test_channel_num, 1] = relerr_rm
                results[i_lamdba, test_channel_num, 2] = relerr_const
                
                mape_results[i_lamdba, test_channel_num, 0] = mape_pwa
                mape_results[i_lamdba, test_channel_num, 1] = mape_rm
                mape_results[i_lamdba, test_channel_num, 2] = mape_const
                
                

case_name = "add_foliage_add_diffraction"

df_root = "./ray_tracing_data/"+case_name+"/Beijing_1.0_fix.csv"
ref_df_root = "./ray_tracing_data/"+case_name+"/Beijing_ref_fix.csv"
lambda_value = ['2.0', '5.0', '10.0', '50.0', '100.0']
n_test = 43 # the number of Tx-Rx pairs

# 0 for PWA, 1 for RM, 2 for Constant Model
results = np.empty([len(lambda_value), n_test, 3]) 
results[:] = np.nan
mape_results = np.empty([len(lambda_value), n_test, 3]) 
mape_results[:] = np.nan

for i in range(len(lambda_value)):
    test_df_root = "./ray_tracing_data/"+case_name+"/Beijing_" + lambda_value[i] +"_fix.csv"
    for i_test in range(n_test):
        test_channel_num = int(i_test)
        print(f'{lambda_value[i]}:{i_test}')
        test_channl(df_root, ref_df_root, test_df_root, i_test, i, results, mape_results)   

# Save as npy
np.save('rel_err_results_'+ case_name, results)
np.save('mape_results_'+ case_name, mape_results)
# Save as CSV
df = pd.DataFrame(columns = ['lambda_dist','link_id','RM_error','PWA_error','ConstM_error'])
lambda_dist_ls = ['2.0', '5.0', '10.0', '50.0', '100.0']
nlambda = 5
ntest = 43
for ilambda in range(nlambda):
    for itest in range(ntest):
        df = df.append({'lambda_dist': lambda_dist_ls[ilambda], 
                        'link_id': itest, \
                       'PWA_error' : results[ilambda, itest, 0],\
                       'RM_error' : results[ilambda, itest, 1],\
                       'ConstM_error' : results[ilambda, itest, 2],\
                        'PWA_mape_sample' : mape_results[ilambda, itest, 0],\
                        'RM_mape_sample' : mape_results[ilambda, itest, 1],\
                        'ConstM_mape_sample' : mape_results[ilambda, itest, 2],\
                           }, ignore_index=True)

df.to_csv('new_rel_err_results_'+ case_name +'.csv',index = False)
